...
let
  ...
  dircolorsCfg = pkgs.writeText "dir_colors" (concatStringsSep "\n"
  ...
  config = mkIf cfg.enable {
    programs = {
      ...
      bash.initExtra = mkIf cfg.enableBashIntegration ''
        eval $(${pkgs.coreutils}/bin/dircolors -b ${dircolorsCfg})
      '';

      fish.shellInit = mkIf cfg.enableFishIntegration ''
        eval (${pkgs.coreutils}/bin/dircolors -c ${dircolorsCfg})
      '';

      # Set `LS_COLORS` before Oh My Zsh and `initExtra`.
      zsh.initExtraBeforeCompInit = mkIf cfg.enableZshIntegration ''
        eval $(${pkgs.coreutils}/bin/dircolors -b ${dircolorsCfg})
      '';

      nushell.extraEnv = mkIf cfg.enableNushellIntegration ''
        source ${
          pkgs.runCommand "dircolors.nu" { } ''
            echo "$(${lib.getExe config.programs.nushell.package} -c "
              source ${pkgs.callPackage ./nushell/capture-foreign-env.nix { }}
              ${
                lib.getExe' pkgs.coreutils "dircolors"
              } -b ${dircolorsCfg} | capture-foreign-env | to nuon
            ") | load-env" >> "$out"
          ''
        }
      '';
